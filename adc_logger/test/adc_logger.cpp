...
int main(){
    ...
    sleep_ms(3000);
    printf("ADCDIV: %.f", static_cast<float>(ADCCLK/ADC_FS));
    ...
    adc_run(1);
    while(1){
        //we will wait until we got a command that ask to check get some data
        //scanf("%s", word);
        //if(strcmp(word
        /*msg = getchar();
        int adc_print = 0;
        ...
        */
        ...
        for(int i=0; i<samples; ++i){
            adc0_data[i] = capture_buffer[3*i];
            adc1_data[i] = capture_buffer[3*i+1];
            adc2_data[i] = capture_buffer[3*i+2];
            //printf("%i %i %i\n", adc0_data[i], adc1_data[i], adc2_data[i]);
            printf("%f %f %f\n", adc0_data[i]*conversion_factor,
                                 adc1_data[i]*conversion_factor,
                                 adc2_data[i]*conversion_factor);
        }
        printf("\n");
        adc_fifo_drain();
        dma_channel_start(control_dma);
